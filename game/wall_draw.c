...
/*   Updated: 2024/12/23 16:33:17 by soksak           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_wall(t_data *data, int x)
{
	...
	step = 1.0 * data->wall->height[data->wall->direction] / data->rc->line_height;
	texPos = (data->rc->draw_start - SHEIGHT / 2 + data->rc->line_height / 2) * step;
	while (y < data->rc->draw_end)
	{
		texY = (int)texPos & (data->wall->height[data->wall->direction] - 1);
		texPos += step;
		color = *(int *)(data->wall->addr[data->wall->direction] +
						 (texY * data->wall->llength[data->wall->direction]) +
						 (data->wall->tex_x * (data->wall->bpp[data->wall->direction] / 8)));
